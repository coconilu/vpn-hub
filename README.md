...
| Guardian | 多目标检测订阅 A 与 `16666`，应用运行期间默认每 180 秒执行一次 |
| 历史展示 | 出口汇总、延迟趋势、稳定状态事件和 Controller 确认切换 |
| Mihomo | 只启动/停止本应用自己创建的 `36666` 开发进程；Controller 仅 loopback 且使用随机 secret |
| 双出口路由 | 订阅 A + 超实惠；优先级、最低延迟、手动均驱动真实选择器，全部失败时 `REJECT` |
| SpeedCat | `26666` 迁移仍未开始，本阶段继续保护其 `6666` 正式入口 |
