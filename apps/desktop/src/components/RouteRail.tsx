...
export function RouteRail({ snapshot }: { snapshot: DashboardSnapshot }) {
  ...
  const target = current === "fail-closed"
    ? "Fail Closed"
    : snapshot.routing.outlets.find((outlet) => outlet.outlet_id === current)?.label ?? current ?? "等待首次健康决策";
  return (
    <section className="route-section" aria-labelledby="route-title">
      ...
      <div className={`route-rail ${active ? "is-active" : ""}`}>
        ...
        <div className="route-edge"><span>{snapshot.entry.host}:{snapshot.entry.port}</span></div>
