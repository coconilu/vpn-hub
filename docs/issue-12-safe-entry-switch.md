...
| 端口冲突 | 预览先做 TCP 快照；提交前再次检查。只有空闲端口或当前 generation 的 exact-owned core 可继续，unknown/third-party 一律拒绝且不终止进程 |
...
| 资产 | 处理方式 |
|---|---|
| 一次性授权 | 仅在进程内存中保存，不通过 IPC 返回密钥；使用后立即消费，过期或设置指纹变化即失效 |
...
| 启动恢复 | 若设置已回滚，只在当前代理仍等于本事务写入值时恢复原快照；若观察到第三方新值，拒绝覆盖并保留证据 |
...
4. 如果期间出现第三方值，绝不覆盖它，事务进入显式恢复失败或下次启动恢复。
...
自动化测试不得读取或修改真实系统代理，也不得触碰用户正在使用的 `6666`。最终人工验收应先将 SpeedCat 移到 `26666`，再从 VPN Hub 的“安全入口切换”执行，并在每一步核对 Controller、全部出口、Fail Closed 和系统代理读回值。
