...
| 选择订阅 A 或 Local client 作为 VPN Hub 出口 | 能，通过 loopback Controller |
| 要求 Local client 在其内部切换节点 | 不能；没有经过验证的公开接口 |
...
2026-07-18 至 2026-07-19 的受控现场验收已确认：用户可通过 v0.2 密码框在本机保存真实订阅，私密文件 ACL、provider cache 和 Controller 启动符合约束；优先级模式可经订阅 A 从 `36666` 访问外网；手动模式可经 Local client 从 `36666` 访问外网；最低延迟模式的迟滞、`16666` 故障切换与三次成功恢复、手动出口不可用时 Fail Closed 与恢复均有 Controller、历史或端到端请求证据。完整脱敏证据见 [Issue #3 受控现场验收：真实订阅与双出口](compatibility/2026-07-18-issue-3-live-subscription.md)。

| 现场验收项 | 状态 |
|---|---|
| 本机私密订阅、ACL、provider cache、Controller ready | 已确认 |
| 优先级模式选择订阅 A，并经 `36666` 发起真实外网请求 | 已确认 |
| 手动模式选择 Local client，Controller/历史一致，并经 `36666` 发起真实外网请求 | 已确认 |
...
本阶段不包含 Local client B `26666`、`6666` 接管、TUN、完整 UDP、Windows Service、托盘、签名或长连接无缝迁移。
