...
2026-07-18 的受控现场验收已确认：用户可通过 v0.2 密码框在本机保存真实订阅，私密文件 ACL、provider cache 和 Controller 启动符合约束；优先级模式可经订阅 A 从 `36666` 访问外网；手动模式可经超实惠从 `36666` 访问外网，并有 UI、Controller 和切换历史的相互印证。完整脱敏证据见 [Issue #3 受控现场验收：真实订阅与双出口](compatibility/2026-07-18-issue-3-live-subscription.md)。
...
| 最低延迟模式的 UI 与 Controller 实际选择 | 待验收 |
| 订阅 A 失败后的真实切换 | 待验收 |
| `16666` 失败后的真实切换 | 待验收 |
| 出口恢复、阈值与冷却回切 | 待验收 |
| 全部出口不可用时现场 Fail Closed | 待验收 |

最低延迟模式的现场 UI 自动化曾误指向 Codex 窗口句柄，发现后已安全停止，因此不能把该操作计为验证通过。订阅凭据不得通过终端参数、CI、Issue、PR 或测试日志注入；若服务端返回的不是 Mihomo/Clash proxy-provider 兼容格式，开发核心会保持 Fail Closed，不能把兼容性伪报为通过。

在最低延迟模式与双出口故障注入完成或明确拆分到后续 Issue 前，Issue #3 保持 open，PR 保持 Draft；自动化通过不代表这些现场验收项已经完成。
