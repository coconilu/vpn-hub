...
- `commit_decided` 之后：保留已提交配置，幂等完成凭据删除与 retention；
...
普通设置没有修改系统代理、防火墙、TUN、Service 或第三方客户端的 API，也没有 DIRECT 故障策略。运行中需要改变 Mihomo 的设置会在任何持久化写入前被拒绝，当前核心和最后有效配置继续工作。
