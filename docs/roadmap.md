...
| 双出口与真实模式 | 实现完成/隔离验收已拆分 | 订阅 A + 超实惠、loopback Controller、三种模式、`16666` 故障切换与手动 Fail Closed 已验收；剩余两项由 Issue #5 跟踪，PR 仍待独立复审 |
...
| 正式 6666 接管 | 明确延期 | 用户仍在使用现有 SpeedCat；当前代码没有接管命令 |
...
- [ ] 测量每个出口的连接建立时间、延迟、连续失败和恢复行为；超实惠侧已完成，订阅 A 故障与双出口 all-down 已拆分到 Issue #5。
