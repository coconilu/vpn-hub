...
- 当前稳定出口 ID 和脱敏显示名称；首页和历史仍不显示订阅节点。
...
- 删除或停用当前活动出口时，情境确认要求选择仍启用的替代出口或明确进入 Fail Closed；不提供允许直连。
- 系统代理只通过专用安全入口切换事务显式选择；当前设置页不提供 TUN 入口。
- 数据保存周期。

### 11.4 订阅节点选择页

- 仅显示已启用且凭据可用的订阅出口，不显示本地黑盒出口的内部节点。
- 节点名称、类型、健康状态和 Mihomo 已知延迟只从本机 Controller 读取并停留在运行时 UI。
- 选择前确认候选仍属于订阅 selector，选择后重新读取 `now`；失败时保留原选择。
- 不展示节点 server/port/认证字段，不把节点名称写入配置、SQLite、普通日志或公开证据。
- 详细边界和状态模型见 [Issue #29 订阅节点选择](issue-29-subscription-node-selection.md)。
